...
fn get_all_days_data() -> (usize, usize, Vec<Matrix>, Vec<Matrix>) {
    ...
    let res = client.into_result(test.0.column(0).as_slice().unwrap());
...
fn initial_training(infos: (usize, usize, &[Matrix], &[Matrix]), batch_size: usize, epoch: i32, lr: f64, momemtum: f64, dir: &str) {
    ...
    let hidden_layer_neurons = 128; //256;
...
fn main() {
    ...
    let (feat_nrows, lab_nrows, new_features, new_labels) = get_all_days_data();
