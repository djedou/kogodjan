...
impl FeedForward {
    ...
    fn training_forward(&mut self, inputs: &Matrix, targets: Matrix, lr: f64) -> f64 {
        ...
        // Update
        for j in 0..self.network_layers.len(){
            self.network_layers[j].update_parameters(lr);
        }
    ...
    fn train(&mut self, network_inputs: &Matrix, network_outputs: &Matrix, lr: f64, batch_size: usize, epoch: i32) {
        for round in 0..epoch {
            ...
            for (feature, label) in data_iter_v2(batch_size, &network_inputs, &network_outputs) {
                ...
                let loss = self.training_forward(&feature, label, lr);
                erros.push(loss);
            }
            
            let loss = self.training_forward(&network_inputs, network_outputs.to_owned(), lr);
