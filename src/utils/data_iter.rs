...
pub fn data_iter_v2(batch_size: usize, features: &Matrix, labels: &Matrix) -> Vec<(Matrix, Matrix)> {
    let feat_nrows = features.nrows();
    let lab_nrows = labels.nrows();
    
    let mut features_chunks = vec![];
    let mut labels_chunks = vec![];

    for chunk in features.exact_chunks((feat_nrows, batch_size)) {
        features_chunks.push(chunk.to_owned());
    }

    for chunk in labels.exact_chunks((lab_nrows, batch_size)) {
        labels_chunks.push(chunk.to_owned());
    }
    
    features_chunks
        .iter()
        .zip(labels_chunks.iter())
        .map(|(i, o)| (i.clone(), o.clone()))
        .collect()
} 


pub fn data_iter_v3(batch_size: usize, features: &[Matrix], labels: &[Vec<Matrix>]) -> Vec<(Matrix, Vec<Matrix>)> {
    let mut rng = thread_rng();
    let num_examples = features.len();
    let feat_nrows = features[0].nrows();
